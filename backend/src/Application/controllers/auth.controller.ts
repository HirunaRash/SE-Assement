...
const errorStatus = (error: unknown, fallback: number): number => {
  ...
  if (prismaCode === 'P2002') return 409;
  return fallback;
};
...
export const register = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await authService.register(normalizedEmail, password, firstName.trim(), lastName.trim(), normalizedRole);
...
export const me = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(200).json({ data: await authService.getUserWithRoles(req.userId) });
  } catch (error) {
