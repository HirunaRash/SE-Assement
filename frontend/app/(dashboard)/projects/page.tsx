...
type Project = {
	...
	createdAt: string;
	memberCount: number;
	project_team_members?: Array<{ userId: number; users?: { firstName: string; lastName: string; email: string } }>;
};

type ProjectResponse = Project & { teamCount?: number; _count?: { reports?: number } };
...
export default function ProjectsPage() {
	...
	const fetchProjects = async () => {
		...
		try {
			...
			setProjects(data.map((project) => ({ ...project, memberCount: project.memberCount ?? project.teamCount ?? project.project_team_members?.length ?? 0 })));
	...
	useEffect(() => {
		...
		fetchProjects();
		api.get('/analytics/submission-by-user').then((response: any) => {
			const members = Array.isArray(response) ? response : [];
			setTeamMembers(members.map((member: any) => ({ id: member.userId, name: member.name || member.userName, email: member.email || '' })));
	...
	const assignMember = async () => {
		...
		setError(null);
		try {
			await api.post(`/projects/${selectedProjectId}/team-members`, { userId: Number(selectedMemberId) });
			setSuccessMessage('Team member assigned successfully');
			setSelectedMemberId('');
			await fetchProjects();
		} catch (requestError: any) {
			setError(requestError.message || 'Unable to assign team member');
		} finally {
	...
	return (
		<main className="min-h-screen bg-black p-4 sm:p-6 lg:p-8">
			...
			{showModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4"><button type="button" aria-label="Close project form" className="fixed inset-0 cursor-default" onClick={closeModal} /><div className="relative z-[51] w-full max-w-md rounded-2xl border border-neutral-800 bg-neutral-900 p-8"><h2 className="mb-8 text-2xl font-bold text-white">{isEditing ? 'Edit Project' : 'Create New Project'}</h2>{error && <div className="mb-6 rounded-lg border border-red-700/50 bg-red-900/20 p-4"><p className="text-sm text-red-400">{error}</p></div>}<form onSubmit={handleFormSubmit} className="space-y-6"><label className="block text-sm font-semibold uppercase tracking-wide text-white">Project Name<input type="text" value={formData.name} onChange={(event) => setFormData({ ...formData, name: event.target.value })} placeholder="e.g., Client A, Internal Tooling, R&D" disabled={formLoading} required className={`mt-2 ${inputClass}`} /></label><label className="block text-sm font-semibold uppercase tracking-wide text-white">Description (Optional)<textarea value={formData.description} onChange={(event) => setFormData({ ...formData, description: event.target.value })} placeholder="Describe this project or category..." disabled={formLoading} className={`mt-2 h-24 resize-none ${inputClass}`} /></label></form>{isEditing && <div className="mt-6 border-t border-neutral-800 pt-6"><label className="block text-sm font-semibold uppercase tracking-wide text-white">Assign Team Member<select value={selectedMemberId} onChange={(event) => setSelectedMemberId(event.target.value)} disabled={assignLoading} className={`mt-2 ${inputClass}`}><option value="">Select a team member...</option>{teamMembers.map((member) => <option key={member.id} value={member.id}>{member.name}{member.email ? ` (${member.email})` : ''}</option>)}</select></label><button type="button" onClick={assignMember} disabled={assignLoading || !selectedMemberId} className="mt-3 w-full rounded-lg bg-blue-600 px-4 py-3 font-bold text-white transition hover:bg-blue-700 disabled:opacity-50">{assignLoading ? 'Assigning...' : 'Assign Member'}</button></div>}<div className="mt-8 flex gap-4"><button type="button" onClick={closeModal} disabled={formLoading || assignLoading} className="flex-1 rounded-lg bg-neutral-800 px-4 py-3 text-white transition hover:bg-neutral-700 disabled:opacity-50">Cancel</button><button type="button" onClick={(event) => handleFormSubmit(event as unknown as FormEvent<HTMLFormElement>)} disabled={formLoading || !formData.name.trim()} className="flex-1 rounded-lg bg-blue-600 px-4 py-3 font-bold text-white transition hover:bg-blue-700 disabled:opacity-50">{formLoading ? (isEditing ? 'Saving...' : 'Creating...') : (isEditing ? 'Save Changes' : 'Create Project')}</button></div></div></div>}
		</main>
	);
}
